...
sdk_package="${package_output}/LibreWPF.Sdk.0.1.0-preview.64.nupkg"
...
apphost_name="ProGPU.Wpf.ShowcaseApp"
...
if [[ "${PROGPU_WPF_SHOWCASE_REBUILD_PACKAGES:-0}" == "1" || ! -f "${sdk_package}" ]]; then
  echo "Building ProGPU WPF SDK packages before launching Showcase app..."
...
if [[ "${PROGPU_WPF_SHOWCASE_SKIP_BUILD:-0}" != "1" ]]; then
  rm -rf \
    "${repo_root}/artifacts/bin/ProGPU.Wpf.ShowcaseApp" \
    "${repo_root}/artifacts/obj/ProGPU.Wpf.ShowcaseApp" \
    "${repo_root}/artifacts/nuget/ProGPU.Wpf.ShowcaseApp"

  echo "Building ProGPU WPF Showcase app..."
  "${dotnet}" build "${showcase_project}" -v:minimal
elif [[ ! -x "${showcase_output}/${apphost_name}" ]]; then
  echo "Expected prebuilt Showcase apphost at ${showcase_output}/${apphost_name}." >&2
  exit 1
fi

if [[ "${PROGPU_WPF_SHOWCASE_LIVE_VALIDATE:-0}" == "1" ]]; then
  export PROGPU_WPF_SHOWCASE_LIVE_VALIDATE
  if [[ "${PROGPU_WPF_SHOWCASE_TRACE_RENDER_SURFACE:-0}" == "1" ]]; then
    export PROGPU_WPF_TRACE_RENDER_SURFACE=1
  fi
  live_log="$(mktemp "${TMPDIR:-/tmp}/progpu-wpf-showcase-live.XXXXXX")"
  live_status="$(mktemp "${TMPDIR:-/tmp}/progpu-wpf-showcase-live-status.XXXXXX")"
  apphost_pid=""
  live_validation_timeout_seconds="${PROGPU_WPF_SHOWCASE_LIVE_VALIDATE_TIMEOUT_SECONDS:-180}"
  if [[ ! "${live_validation_timeout_seconds}" =~ ^[1-9][0-9]*$ ]]; then
    echo "Invalid PROGPU_WPF_SHOWCASE_LIVE_VALIDATE_TIMEOUT_SECONDS value '${live_validation_timeout_seconds}'." >&2
    exit 1
  fi
  cleanup_live_probe() {
  ...
  echo "Launching ProGPU WPF Showcase apphost live geometry probe..."
  (
    cd "${showcase_output}"
    if (($# > 0)); then
      PROGPU_WPF_SHOWCASE_LIVE_VALIDATE_STATUS_PATH="${live_status}" "./${apphost_name}" "$@"
    else
      PROGPU_WPF_SHOWCASE_LIVE_VALIDATE_STATUS_PATH="${live_status}" "./${apphost_name}"
    fi
  ) >"${live_log}" 2>&1 &
  ...
  while (( SECONDS < live_validation_deadline )); do
    live_validation_line="$(grep -h -E "ProGPU WPF Showcase live input validation succeeded:" "${live_status}" "${live_log}" 2>/dev/null | tail -n 1 || true)"
    ...
    if ! kill -0 "${apphost_pid}" 2>/dev/null; then
      ...
      echo "Showcase apphost exited before live input validation succeeded." >&2
  ...
  if [[ -z "${live_validation_line}" ]]; then
    echo "Expected Showcase apphost live input validation to succeed before timeout." >&2
    if [[ -f "${live_status}" ]]; then
      cat "${live_status}" >&2
    fi
    cat "${live_log}" >&2
  ...
  if [[ -n "${render_surface_line}" ]]; then
    if [[ ! "${render_surface_line}" =~ logical[[:space:]]([0-9]+)x([0-9]+),[[:space:]]pixels[[:space:]]([0-9]+)x([0-9]+),[[:space:]]viewport[[:space:]]([0-9]+)x([0-9]+)@([0-9]+),([0-9]+),[[:space:]]dpi[[:space:]]([0-9]+(\.[0-9]+)?) ]]; then
      echo "Could not parse Showcase apphost render-surface line: ${render_surface_line}" >&2
      cat "${live_log}" >&2
      exit 1
    fi
    ...
    echo "Could not parse Showcase apphost geometry from live validation or render-surface line." >&2
    cat "${live_log}" >&2
    exit 1
  fi
  ...
  if (( pixel_width < logical_width || pixel_height < logical_height )); then
    echo "Expected Showcase apphost pixels to cover ${logical_width}x${logical_height} logical content, but got ${pixel_width}x${pixel_height}." >&2
    cat "${live_log}" >&2
    exit 1
  fi
  ...
  if (( has_viewport_geometry == 1 )); then
    echo "ProGPU WPF Showcase live geometry validation succeeded: logical ${logical_width}x${logical_height}, pixels ${pixel_width}x${pixel_height}, viewport ${viewport_width}x${viewport_height}@${viewport_x},${viewport_y}."
  else
    echo "ProGPU WPF Showcase live geometry validation succeeded: logical ${logical_width}x${logical_height}, pixels ${pixel_width}x${pixel_height}."
  fi
  cleanup_live_probe >/dev/null 2>&1
  exit 0
fi

echo "Launching ProGPU WPF Showcase apphost..."
(
  cd "${showcase_output}"
  if (($# > 0)); then
    "./${apphost_name}" "$@"
