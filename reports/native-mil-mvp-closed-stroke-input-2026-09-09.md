# MVP closed stroked path input — 2026-09-09

## Acceptance dependency and implementation

The package MVP's `MvpShapePath` has the actual contour
...
batch. Scene 9815 uses canonical MIL for the actual MVP fill-plus-stroke path.
...
The next concrete input blocker is the same MVP's `MvpBlurEffectBorder` and
`MvpDropShadowEffectBorder` in the EventTrigger tab. Their source `Visual` input
