...
| 2 — current | Application closure: use the same MVP, Toolkit/AvalonDock, license-controlled Xceed and existing SciChart gate. Current concrete input blocker: native MIL emits no retained GPU hit-test index, while host owner queries still use the managed index. Immutable source-owner snapshots and compositor/scene-qualified query tokens are now connected, but native index emission and host query routing remain open. Finish required text/selection, scroll/clip, popup/input, resize/DPI, content/effect/cache updates and close/reopen/device-loss ownership. Reuse existing implementations; fix concrete missing connections. | Each required action has an implemented path and authored regression coverage. Record any known blocking branch against that action; do not reopen already connected subsystems for optional refinements. |
