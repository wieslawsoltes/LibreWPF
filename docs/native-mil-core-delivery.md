...
| 2 — current | Application closure: use the same MVP, Toolkit/AvalonDock, license-controlled Xceed and existing SciChart gate. Current concrete input blocker: native MIL has opt-in index emission for analytic primitives/plain path fills, but lacks complete application hit coverage; host owner queries still use the managed index. Immutable source-owner snapshots and compositor/scene-qualified query tokens are connected. Complete text/image/stroke/clip/cache/effect hit coverage and native host routing before enabling the option. Finish required text/selection, scroll/clip, popup/input, resize/DPI, content/effect/cache updates and close/reopen/device-loss ownership. Reuse existing implementations; fix concrete missing connections. | Each required action has an implemented path and authored regression coverage. Record any known blocking branch against that action; do not reopen already connected subsystems for optional refinements. |
