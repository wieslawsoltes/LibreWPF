...
and `IDWriteFontFace*` types.

The next ProGPU increment implements `DrawTextLayout` through a canonical
`IDWritePixelSnapping`/`IDWriteTextRenderer` callback object. Existing system
or portable layouts synchronously emit glyph runs, underlines,
strikethroughs, and inline objects into the same target. Glyphs reuse the
complete-run retained path, decorations lower to analytic rectangles, inline
objects receive the same renderer recursively, and typed `ID2D1Brush` drawing
effects may override the default brush. `NO_SNAP` and the current transform/DPI
flow through the callback; `CLIP` reads the layout's ABI-stable maximum size
...
partitions independent feature groups with `CanvasDrawingSession.Flush()` so
one CPU-D3D12 batch does not contain the complete path/text/layer workload.
Every original draw and pixel probe remains under automatic GPU-first
selection, with no intermediate readback or CPU composition. This exposed and
fixed preserved incremental Canvas commits that contained isolated layers:
the managed backend requests typed full-target preservation and the native root
pass loads the existing attachment. Partitioned Metal is byte-identical to the
qualified full frame and reports all work as 17+2 native draws after the extra
...
locally; cross-platform PR CI and downstream package application runs are still
required at immutable merged heads.
