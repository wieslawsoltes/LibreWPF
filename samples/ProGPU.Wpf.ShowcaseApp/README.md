# LibreWPF Showcase App

This showcase exercises the LibreWPF SDK using ordinary WPF application source. It uses regular WPF XAML and code-behind with:
...
```bash
PROGPU_WPF_SHOWCASE_RUN_VALIDATE=1 ./eng/run-progpu-wpf-showcase.sh
```
...
If the local `0.1.0-preview.64` LibreWPF packages are stale or missing, rebuild the SDK package feed first:

```bash
PROGPU_WPF_SHOWCASE_REBUILD_PACKAGES=1 ./eng/run-progpu-wpf-showcase.sh
```
